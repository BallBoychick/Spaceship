...
public class TestExceptionFindHandler
{
    ...
    public TestExceptionFindHandler()
    {
        ...
        mockStrategy.Setup(x => x.RunStrategy()).Returns("it's strategy");

        Mock<IStrategy> mockNotCommandStrategy = new Mock<IStrategy>();
        mockNotCommandStrategy.Setup(x => x.RunStrategy()).Returns("it's not found command strategy");

        Mock<IStrategy> mockNotExceptionStrategy = new Mock<IStrategy>();
        mockNotExceptionStrategy.Setup(x => x.RunStrategy()).Returns("it's not found exception strategy");
